...
RSpec.describe 'As an admin user' do
  describe 'when i click a new merchants link' do
    before :each do
      ...
      @merchant_1_location = @merchant_1.locations.create!(name: 'home', address: "1234 Test Rd", city: "Kansas City", state: "MO", zip: '64086')
    ...
    it 'shows data from the merchants show page' do
      ...
      expect(page).to have_content(@merchant_1_location.address)
      expect(page).to have_content(@merchant_1_location.state)
      expect(page).to have_content(@merchant_1_location.zip)
    end
